...
```{r}
ggplot(nes, aes(x = income, y = dvote)) +
  geom_jitter(alpha = 0.5, height = 0.4) +
...
The maximum predicted decrease in probability of voting for Democrats due to someone being male is roughly `r -0.159/4`. The maximum decrease due to someone moving to a higher income bracket is roughly `r -0.288/4`.
...
In the Bayesian interpretation, the true logistic coefficient for income has a 95% chance of being within the bounds of the confidence interval. In the Frequentist interpretation, if we took infinitely many samples, the coefficient for income in 95% of them would be within the interval.
...
People in higher income brackets are less likely to vote for Democratic presidential candidates.
...
The prdicted probabilities of voting for democratic presidential candidates for individual observations within the dataset. 
...
model_1 %>%
  ...
  unique()
```
Since our model only takes a binary gender variable and an income bracket variable with 5 levels into account, it makes sense that it predicts the same probability of voting for a democrat for all males at a certain income level.
...
```{r}
new_predictions <- model_1 %>%
  augment(type.predict = "response", 
          newdata = tibble(income = rep(mean(nes$income), 2), 
...
```{r}
new_predictions %>%
  ggplot(aes(x = gender, y = .fitted, ymin = conf.low, ymax = conf.high)) +
  geom_point(size = 3) +
  geom_errorbar(color = "red") +
  theme_classic() +
  labs(title = "Probability of Voting for a Democratic Presidential Candidate",
       subtitle = "For average-income voters",
       x = "Gender",
      y = "Predicted probability of voting for Democrats")
```
...
```{r}
new_predictions %>%
  mutate(predicted_vote = ifelse(.fitted >= 0.5, "Democrat", "Republican"))
```
